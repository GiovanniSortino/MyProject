...
def createModel():
    base_model = VGG16(weights='imagenet', include_top=False) #Con include_top non prendo la parte di FNN o fully connected ma solo la prima parte convoluzionale
    model = Model(inputs=base_model.input, outputs=base_model.get_layer('block5_pool').output) #Prendo l'uscita dell'ultimo layer convoluzionali
    return model

def preprocessing(image):
    #Aggiunge una dimensinoe perchè è richiesta dalla VGG
